...
```{r}
logit.mod <- glm(y ~ Safety.Score + Average.Student.Attendance + ISAT.Exceeding.Reading.. + College.Enrollment..number.of.students., data = numeric.data1, family = "binomial")
summary(logit.mod)
```

```{r}
gstatistic = 359.2 - 145.7
df = 4
1 - pchisq(gstatistic, df)
```
...
confusion(round(logit.mod$fitted.values), (numeric.data1$y), quietly = F)
```
