...
newX <- model.matrix(~.-Average.Student.Attendance, data=test.set)
...
MSE.lasso = mean((lasso.predictions - test.set$Average.Student.Attendance)^2)
MSE.ridge = mean((ridge.predictions - test.set$Average.Student.Attendance)^2)
...
data = gather(data.frame(Full_Model = full.error,
                  ...
                  Lasso = lasso.error,
                  Ridge = ridge.error), key = "Method", value = "Error")
ggplot(data) + 
  geom_boxplot(aes(x = Method, y = Error, fill = Method))
```
